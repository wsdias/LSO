...
#define  DEVICE_NAME "simple_driver_modified" ///< The device will appear at /dev/simple_driver using this value
#define  CLASS_NAME  "simple_class_modified"        ///< The device class -- this is a character device driver
...
static struct elem_labsisop *message;
 ...
 */
static int __init simple_init_modified(void){
 ...
 */
static void __exit simple_exit_modified(void){
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
    ...
    error_count = copy_to_user(buffer, message->param, size_of_message);
    list_for_each_entry(message, &message_list, node){
        printk("%s",message->param);
        //list_del() ...
    }
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
    ...
    message = kzalloc(sizeof(struct elem_labsisop), GFP_KERNEL);
    sprintf(message->param, "%s", buffer);
    message->value = len;
    list_add_tail(&message->node, &message_list);
 ...
 *  listed above)
 */
module_init(simple_init_modified);
module_exit(simple_exit_modified);
